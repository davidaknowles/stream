...
- Benchmarked UCE as a replacement cell-state representation. The official 33-layer checkpoint loaded strictly into the reference architecture (1,280 hidden dimensions, 20 attention heads, 5,120-dimensional feed-forward layers). With 1,024 sampled genes plus UCE structural tokens (sequence length 1,045), the checkpoint-backed batch-50 measurement sustained 27.9 cells/s. This projects to 113.9 GPU-hours for all 11,441,407 atlas cells on one GPU. Batch sizes 25, 50, and 100 measured 28.5, 29.6, and 28.1 cells/s respectively in short architecture-only runs; batch 50 is the practical setting for the initial run.
