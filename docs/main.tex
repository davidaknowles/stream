...
\uce{} embedding computed from that expression state. The output is always a $G$-dimensional expression velocity,
...
\right],
\end{equation}
where $s_\tau=x_\tau$ for expression-state runs and $s_\tau=E(x_\tau)$ for
online-\uce{} runs. Training pairs are sampled from an entropic minibatch
...
\begin{equation}
u_\tau=E\left((1-\tau)x_k+\tau x_{k+1}\right).
\end{equation}
The target $\hat v$, prediction, and loss remain expression-valued. During
rollout, \uce{} is recomputed after every expression update, yielding the closed
autonomous system $dx/dt=f_\theta(E(x),\{\mathbf Z_g\})$. No destination
embedding or explicit time input is supplied.
