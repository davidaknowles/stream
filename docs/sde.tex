...
\section{Gene-scaled dynamical coordinates}

Let $q\in\R^G_{>0}$ be fixed per-gene scales estimated from raw training
counts, $Q=\operatorname{diag}(q)$, and
\begin{equation}
  Y=Q^{-1}X,\qquad X=QY.
  \label{eq:scaled-coordinates}
\end{equation}
The implementation performs interpolation, velocity prediction, score
matching, and ODE/SDE integration in $Y$. Before every online \uce{} call and
endpoint metric, it deterministically returns to count-like expression with
$X=QY$. OT and train-only PCA remain in $X$. Because $q_g>0$, nonnegative
projection is equivalent in both coordinate systems.

For a general diffusion,
\begin{equation}
  \dd Y_t=\mu_Y(Y_t)\dd t+G_Y(Y_t)\dd W_t
  \quad\Longleftrightarrow\quad
  \dd X_t=Q\mu_Y(Q^{-1}X_t)\dd t+QG_Y(Q^{-1}X_t)\dd W_t.
  \label{eq:coordinate-sde}
\end{equation}
Thus the previous gene-scaled Gaussian noise $Q\dd W$ becomes isotropic in
$Y$, and an ordinary MSE on $Y$-velocity is the intended gene-normalized loss
in count space. The fixed $q$ is now a checkpointed boundary transform rather
than a factor repeated throughout the model equations.
...
Equations~\ref{eq:stochastic-interpolant}--\ref{eq:coupled-fields} show the
count-space equivalent. With $\Sigma=Q$, the implemented model-coordinate
form is
\begin{align}
  Y_\tau&=(1-\tau)y_0+\tau y_1+\gamma(\tau)z,\\
  b_Y(y,\tau)&=u_Y(y,\tau)
    +\frac{\dot\gamma(\tau)}{\Delta t}e_\theta(y,\tau),\\
  s_Y(y,\tau)&=-\frac{e_\theta(y,\tau)}{\gamma(\tau)}.
  \label{eq:scaled-coupled-fields}
\end{align}
The model receives $E(QY_\tau)$, predicts all gene-valued quantities in $Y$,
and uses $X=QY$ only at UCE and evaluation boundaries.
...
\subsection{Implemented score-flow fine-tuning}

The first explicit-rollout implementation starts from the coupled
score-and-flow checkpoint rather than introducing a new diffusion head. For
each observed training interval it samples source cells and target cells
independently. No balanced, partial, or unbalanced OT plan is used to construct
cell pairs. Each update jointly evaluates the coupled probability-flow ODE
($\kappa=0$) and one score-corrected SDE with
$\kappa\in\{10^{-3},10^{-2}\}$:
\begin{equation}
  X_{k+1}=\Pi_+\!\left[X_k+\Delta t
  \left(v_\theta+\kappa\,\Sigma s_\theta\right)
  +\sqrt{2\kappa\Delta t}\,\Sigma\epsilon_k\right].
  \label{eq:implemented-rollout}
\end{equation}
Here $v_\theta$ and $s_\theta$ retain the shared noise parameterization from
Equation~\ref{eq:score-flow-sde}, and $\Sigma$ is the fixed robust gene scale
estimated during simulation-free fitting.

The regulatory trunk, autonomous velocity head, and frozen \uce{} encoder are
held fixed. Fine-tuning updates only the interpolation-coordinate modulation,
conditional velocity head, and shared noise head, with a quadratic anchor to
their pretrained values. At every integration step, raw current expression is
retokenized and encoded by \uce{}; neither KNN nor metacell smoothing is
applied during rollout. The gradient through this discrete state map is
truncated, while gradients propagate through the additive expression update
and directly into the fields evaluated at each step.

The endpoint objective averages the ODE and SDE losses. Each contains the
debiased Sinkhorn divergence in whitened train-only PCA coordinates, a
gene-mean penalty normalized by robust gene scale, and a PCA-covariance
penalty. The optimized Sinkhorn plan is detached when computing gradients,
which applies the envelope gradient without retaining all matrix-scaling
iterations. Validation uses disjoint cells, fixed source and target batches,
fixed Brownian seeds, and interval-balanced stages. The pretrained checkpoint
is update zero, and an exponential moving average of validation endpoint loss
selects the fine-tuned checkpoint.
...
proliferation and death are central, the implemented extension adds a
growth-rate head $\gamma_\psi(E(x))$ and normalized particle weights
\begin{equation}
  \widetilde w_{k+1,i}=w_{k,i}\exp\!\left(
    [\gamma_\psi(E(X_{k,i}))-\bar\gamma_k]\Delta t\right),
  \qquad
  w_{k+1,i}=\frac{\widetilde w_{k+1,i}}
  {\sum_j\widetilde w_{k+1,j}},
  \label{eq:growth-weights}
\end{equation}
where $\bar\gamma_k=\sum_iw_{k,i}\gamma_\psi(E(X_{k,i}))$. The centered rate
is identifiable from stage-normalized snapshot data; absolute embryo growth
is not. A bounded rate, rate-squared penalty, and KL penalty from the uniform
particle weights limit collapse onto a few cells.

Training replaces the predicted marginal in the endpoint Sinkhorn divergence,
gene mean, and PCA covariance with $w_K$. Gradients through weighted Sinkhorn
matrix scaling reach the growth head. Three causal controls separate the
mechanisms: fixed-mass velocity, growth-only reweighting with $X_k=X_0$, and
joint velocity plus growth. Learned-score and zero-score SDE controls are
retained for the joint model. This distinguishes state motion, stochastic
dispersion, and changing relative lineage abundance.
