...
\subsection{Flow-matching head}

The marginal flow field is learned by conditional regression:
\begin{align}
  b_\theta(x,\tau)
  &\approx \E[\dot X_\tau\mid X_\tau=x], \\
  \dot X_\tau
  &= (x_1-x_0)+\dot\gamma(\tau)z, \\
  \mathcal L_{\mathrm{flow}}
  &= \E\left[
       \left\|b_\theta(X_\tau,\tau)-\dot X_\tau\right\|_2^2
     \right].
  \label{eq:stochastic-flow-loss}
\end{align}
For physical developmental intervals, the target is divided by the interval
duration. As in the current model, $b_{\theta,g}$ is one expression-valued
output per gene.

\subsection{Score-matching head}

The score of the interpolant marginal is
\begin{equation}
  s_\phi(x,\tau)\approx\nabla_x\log p_\tau(x).
\end{equation}
Because Equation~\ref{eq:stochastic-interpolant} adds Gaussian noise, the score
can be trained by denoising regression:
\begin{equation}
  \mathcal L_{\mathrm{score}}
  = \E\left[
      w(\tau)
      \left\|
        s_\phi(X_\tau,\tau)+\frac{z}{\gamma(\tau)}
      \right\|_2^2
    \right].
  \label{eq:score-loss}
\end{equation}
Choosing $w(\tau)=\gamma(\tau)^2$ prevents the endpoint singularity from
dominating optimization. Flow and score heads can share the regulatory token
transformer:
\begin{align}
  b_{\theta,g}(x,\tau)
  &= \operatorname{STREAM}^{\mathrm{flow}}_g
     \!\left(E(x),Z_g,\tau\right), \\
  s_{\phi,g}(x,\tau)
  &= \operatorname{STREAM}^{\mathrm{score}}_g
     \!\left(E(x),Z_g,\tau\right).
\end{align}

\subsection{SDE induced by the matched fields}

If $b_\theta$ satisfies the continuity equation and $s_\phi$ estimates the
marginal score, then the fields define the SDE
\begin{equation}
  \dd X_\tau
  = \left[
      b_\theta(X_\tau,\tau)
      + \kappa(\tau)s_\phi(X_\tau,\tau)
    \right]\dd\tau
    + \sqrt{2\kappa(\tau)}\,\dd W_\tau.
...
\begin{enumerate}
  \item Add shared flow and score heads to the reusable model package, together
        with a local interpolation-coordinate embedding. Keep current
  ...
  \item Add weighted flow and score losses, with tests against analytic
        Gaussian bridges and known marginal scores.
