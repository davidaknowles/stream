...
\midrule
$x,X_t,q,u,d,\widehat d$ & $\R^G$ & Expression state, gene scales, or gene-valued direction/displacement \\
$E(x)$ & $\R^D$ & Frozen \uce{} cell-state representation; currently $D=1280$ \\
...
The previously undefined $q=(q_1,\ldots,q_G)\in\R^G_{>0}$ is a fixed vector
of training-only per-gene expression scales, estimated from endpoint counts
with a nonzero floor. It defines the dimensionless scaled inner product and
norm
\begin{equation}
  \langle y,z\rangle_q
  =G^{-1}\sum_{g=1}^{G}\frac{y_gz_g}{q_g^2},
  \qquad
  \lVert y\rVert_q=\sqrt{\langle y,y\rangle_q}.
  \label{eq:q-norm}
\end{equation}
Thus $q_g$, $x_g$, and $u_g$ have expression units. The latent progress $S$
is dimensionless, $a_\phi$ has units of inverse experimental time, and
$\sigma_\phi$ has units of inverse square-root time. Consequently
$u_\theta\,\dd S$ has expression units. The bridge coordinate $\tau$, bridge
amplitude $\rho$, and clock draw $z$ are dimensionless. Crucially,
$z\in\R$ is one scalar shared across all genes for one cell and path, unlike
the regular score-flow noise $\epsilon\in\R^G$.

\subsection{Latent stochastic clock}

Introduce a scalar biological-progress process $S_t$ and write
\begin{align}
  \dd S_t &= a_\phi(X_t)\,\dd t + \sigma_\phi(X_t)\,\dd W_t^{S},
  \label{eq:clock-sde}\\
  \dd X_t &= u_\theta(X_t)\,\dd S_t
             + G_\psi(X_t)\,\dd W_t^{\perp}.
  \label{eq:expression-sde}
\end{align}
The scalar clock rate and variability are state-dependent,
\begin{equation}
  a_\phi(x)=\operatorname{softplus}\!\left(h_a(E(x))\right),
  \qquad
  \sigma_\phi(x)=\operatorname{softplus}\!\left(h_\sigma(E(x))\right),
...
\begin{equation}
  ...
  = a_\phi(X_k)\Delta t
    \exp\!\left[
      \sigma_\phi(X_k)\sqrt{\Delta t}\,\epsilon_k
      -\frac{1}{2}\sigma_\phi(X_k)^2\Delta t
    \right],
  \qquad \epsilon_k\sim\N(0,1).
  \label{eq:positive-increment}
\end{equation}
The correction in the exponent preserves
$\E[\Delta S_k\mid X_k]=a_\phi(X_k)\Delta t$. The expression update is
\begin{equation}
  X_{k+1}=\Pi_+\!\left[
    X_k+u_\theta(X_k)\Delta S_k
    +G_\psi(X_k)\sqrt{\Delta t}\,\epsilon_k^\perp
  \right],
  \label{eq:clock-rollout}
\end{equation}
where $\Pi_+$ projects expression to the nonnegative orthant. This
reparameterization is differentiable with respect to the model parameters.
...
separate direction from progress, normalize the raw regulatory velocity by a
robust gene-space norm:
\begin{equation}
  u_\theta(x)
  = \frac{v_\theta(x)}{
      \sqrt{G^{-1}\sum_{g=1}^{G}
      \left[v_{\theta,g}(x)/q_g\right]^2}+\epsilon},
  \label{eq:direction-normalization}
\end{equation}
where $q_g$ is a fixed training-only robust gene scale. The clock increment
then determines distance traveled in scaled expression space. A less invasive
...
Clock noise is rank one locally: its expression-space direction is
$u_\theta(x)$. Residual diffusion should not be allowed to relearn the same
component. Given an unconstrained residual draw $r$, project it away from the
scaled velocity direction,
\begin{equation}
  r_\perp
  = r-u\frac{\langle r,u\rangle_q}{\langle u,u\rangle_q},
  \qquad
  \langle y,z\rangle_q
  = G^{-1}\sum_g\frac{y_gz_g}{q_g^2}.
...
population $\widehat{\mathcal X}_1$ with the observed target population
$\mathcal X_1$ using
\begin{align}
  \mathcal L_{\mathrm{endpoint}}
  =&\;S_\varepsilon\!\left(
      P(\widehat{\mathcal X}_1),P(\mathcal X_1)
    \right)
...
\begin{equation}
  d=x_1-x_0,\qquad
  \ell=\lVert d\rVert_q
      =\sqrt{G^{-1}\sum_g(d_g/q_g)^2},\qquad
...
\begin{align}
  X_\tau
  &=(1-\tau)x_0+\tau x_1+\rho\gamma(\tau)\widehat d\,z,
  &z&\sim\N(0,1),
...
gene. Here $\rho>0$ is a fixed dimensionless bridge-noise amplitude, analogous
to the fixed noise amplitude in the current stochastic interpolant. The model
predicts a normalized direction $u_\theta(X_\tau)$, positive rate
$a_\phi(X_\tau)$, and scalar noise
$\widehat z_\psi(X_\tau,\tau)\in\R$. The rollout clock scale
$\sigma_\phi(X_\tau)$ is subsequently calibrated and fine-tuned by the
...
same scalar estimate. Define the dual scaled direction
\begin{equation}
  (u_q^*)_g=\frac{u_g}{Gq_g^2},
  \qquad
  u_q^{*T}u=\lVert u\rVert_q^2=1.
...
\begin{equation}
  ...
  =\mathcal L_{\mathrm{flow}}
   ...
   +\lambda_d\left[1-\cos_q(u_\theta,\widehat d)\right]
...
\midrule
A & $\dd X=v_\theta\dd t$ & None & Fixed-clock persistence-relative baseline \\
B & $\dd X=a_\phi u_\theta\dd t$ & Deterministic rate & Does state-dependent speed help? \\
C & $\dd X=u_\theta\dd S$ & Rate and scalar clock noise & Does variable progress improve endpoint prediction? \\
