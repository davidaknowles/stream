...
\end{equation}
where $\Sigma$ contains fixed gene scales. This noise can cause some particles
...
\begin{equation}
  d=x_1-x_0,\qquad
  \ell=\sqrt{G^{-1}\sum_g(d_g/q_g)^2},\qquad
...
\begin{align}
  X_\tau
  &=(1-\tau)x_0+\tau x_1+\gamma(\tau)\widehat d\,z,
  &z&\sim\N(0,1),
  \label{eq:clock-interpolant}\\
  \partial_\tau X_\tau
  &=d+\gamma'(\tau)\widehat d\,z.
...
gene. The model predicts a normalized direction $u_\theta(X_\tau)$, positive
rate $a_\phi(X_\tau)$, clock scale $\sigma_\phi(X_\tau)$, and scalar noise
$\widehat z_\psi(X_\tau,\tau)$. A coupled clock-flow readout has the form
\begin{equation}
  f_{\mathrm{clock}}
  =a_\phi u_\theta
   +\frac{\gamma'(\tau)}{t_1-t_0}
    \sigma_\phi u_\theta\widehat z_\psi,
  \label{eq:coupled-clock-flow}
\end{equation}
while the score component tangent to the trajectory is constructed from the
same scalar estimate,
\begin{equation}
  s_\parallel
  =-\frac{\widehat z_\psi}
          {\gamma(\tau)\sigma_\phi+\epsilon}\,u_\theta.
