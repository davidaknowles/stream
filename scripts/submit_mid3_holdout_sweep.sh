...
include_bridges="${INCLUDE_HELDOUT_BRIDGE_INTERVALS:-0}"
cd "$project_root"
...
for max_skip in 0 1 2; do
  suffix=""
  if [[ "$include_bridges" == "1" || "$include_bridges" == "true" ]]; then suffix="_bridge"; fi
  name="mid3_partial_pcacost_denoise_pca_uce_skip${max_skip}${suffix}"
