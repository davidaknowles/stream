#!/bin/bash
# Submit KNN and metacell endpoint-denoising ablations.
...
neighbors="${DENOISING_NEIGHBORS:-15}"
metacells="${DENOISING_METACELLS:-512}"
uce_preprocessing="${UCE_EXPRESSION_PREPROCESSING:-raw}"
gpu_type="${GPU_TYPE:-b6k}"
smoke_jobs="${DENOISING_SMOKE_JOBS:-}"
cd "$project_root"

[[ -f "$pca_artifact" ]] || { echo "Missing PCA artifact: $pca_artifact" >&2; exit 1; }
dependency=()
if [[ -n "$smoke_jobs" ]]; then
  dependency=(--dependency="afterok:${smoke_jobs}")
fi

for variant in film cross_attention; do
  for denoising in knn metacell; do
    if [[ "$uce_preprocessing" == "raw" ]]; then
      name="bank_partial_pcacost_${denoising}_adj"
    else
      name="bank_partial_pcacost_${denoising}_uce_${uce_preprocessing}_adj"
    fi
    ...
    echo "${variant} ${denoising}: train=${train_job}; evaluation=${eval_job}"
  done
done
