...
dynamics_coordinates="${DYNAMICS_COORDINATES:-count}"
label_version="${LABEL_VERSION:-coupled_v2}"
cd "$project_root"
evaluation_jobs=()

for variant in film cross_attention; do
  for denoising in none knn metacell; do
    label="mid3_partial_pcacost_${denoising}_systematic_${label_version}"
    stem="score_flow_${variant}_${label}"
    train_job="$(sbatch --parsable --gres="gpu:${gpu_type}:1" --job-name="sfc_${variant}_${denoising}" \
      --export="ALL,MODE=train,OUT_DIR=${out_dir},TIMEPOINT_SPLIT=${split},PCA_ARTIFACT=${pca},CRE_TOKEN_ARRAYS=${cre},VARIANT=${variant},ENDPOINT_DENOISING=${denoising},EXPERIMENT_LABEL=${label},DYNAMICS_COORDINATES=${dynamics_coordinates},OT_POOL_SIZE=16384,OT_PAIRS_PER_POOL=1024,BATCH_SIZE=8,GENE_CHUNK_SIZE=256,EPOCHS=50,STEPS_PER_EPOCH=100,STREAM_PYTHON=${python_bin}" \
...
summary_job="$(sbatch --parsable --partition=cpu --cpus-per-task=1 --mem=8G --time=00:30:00 \
  ...
  --wrap="bash -lc 'cd ${project_root} && source ~/.bashrc && ${python_bin} scripts/summarize_score_flow_ablation.py --out-dir ${out_dir} --pattern \"causal_eval_model_score_flow_*_${label_version}.csv\" --output score_flow_${label_version}_ablation_summary.csv'")"
