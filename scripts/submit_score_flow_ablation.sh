#!/bin/bash
# Submit matched raw, KNN, and metacell score-flow experiments.
...
for variant in film cross_attention; do
  for denoising in none knn metacell; do
    label="mid3_partial_pcacost_${denoising}_systematic"
    stem="score_flow_${variant}_${label}"
    train_job="$(sbatch --parsable --gres="gpu:${gpu_type}:1" --job-name="sf_${variant}_${denoising}" \
    ...
    eval_job="$(sbatch --parsable --gres="gpu:${gpu_type}:1" --dependency="afterok:${train_job}" \
      --job-name="sf_${variant}_${denoising}_eval" \
...
summary_job="$(sbatch --parsable --partition=cpu --cpus-per-task=1 --mem=8G --time=00:30:00 \
  --dependency="afterok:${dependency}" --job-name=sf_summary \
  --output=logs/score_flow_summary_%j.out --error=logs/score_flow_summary_%j.err \
